...
const TaskInstanceComponent = ({
   ...
   onRestartTaskButtonClick
    ...
    let transformedStatusFilter = Filters.Status.map((filter) => filter.value)
    if (Filters.Status.length > 0) {
        showTaskInstance = taskStatuses.some(aStatus => transformedStatusFilter.includes(aStatus));
    }
    ...
    if (Filters.Users.length > 0) {
        showTaskInstance = showTaskInstance &&
            Filters.Users.some(filter => filter.value === TaskInstance.User.UserID);
    }
