...
class NotificationsComponent extends React.PureComponent {
    ...
    async dismissNotification() {
        const {NotificationsID} = this.props.Notification;
        const {data} = await axios.post(`/notifications/dismiss/${NotificationsID}`, {});
        ...
        return (
            <div className="notification">
                <div style={{float: 'right', marginTop: '-15px'}}>
                    ...
                    {((this.props.Notification.NotificationTarget === 'Flag') || (this.props.Notification.NotificationTarget === 'Comment')) && (<Link to={`/task/${this.props.Notification.LinkID}?tab=comments&target=${this.props.Notification.CommentTarget}&targetID=${this.props.Notification.CommentTargetID}&commentsID=${this.props.Notification.TargetID}`} ><i style={{padding: 10, color: '#1D578C'}} className="fa fa-arrow-circle-right"/></Link>)}
                    {(((this.props.Notification.NotificationTarget === 'SectionUser') || (this.props.Notification.NotificationTarget === 'VolunteerPool')) && (this.props.Notification.Role === 'Instructor')) && (<Link to={`/course-section-management/?org=${this.props.Notification.OrganizationID}?course=${this.props.Notification.CourseID}&semester=${this.props.Notification.SemesterID}&section=${this.props.Notification.SectionID}&user=${this.props.Notification.ActorID}`}><i style={{padding: 10, color: '#1D578C'}} className="fa fa-arrow-circle-right"/></Link>)}
                    {(((this.props.Notification.NotificationTarget === 'SectionUser') || (this.props.Notification.NotificationTarget === 'VolunteerPool')) && (this.props.Notification.Role === 'Student')) && (<Link to={`/account/?section=${this.props.Notification.SectionID}`}><i style={{padding: 10, color: '#1D578C'}} className="fa fa-arrow-circle-right"/></Link>)}
                    <i style={{padding: 10}} className="fa fa-times" onClick={this.dismissNotification.bind(this)}/>
                </div>
