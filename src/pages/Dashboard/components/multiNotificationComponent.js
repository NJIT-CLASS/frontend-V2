...
class MultiNotificationsComponent extends Component {
    ...
    dismissNotification() {
        for (let i of this.props.MultiNotification) {
            ...
            axios.post(`/notifications/dismiss/${i.NotificationsID}`).then((response) => {
                const { data } = response;
                if (!data.Error) {
    ...
    getAccessLink() {
        axios.get(`/comments/accessLink/${this.props.OriginTaskInstanceID}`).then((response) => {
        ...
        for (let i of this.props.MultiNotification) {
            axios.get(`/comments/CommentsID/${i.CommentsID}`).then((response) => {
                const body = response.data;
                if (!body.Error) {
...
export default MultiNotificationsComponent;
