...
import {TASK_TYPES_TEXT} from "../../constants/TASK_TYPES_TEXTS";
...
class AssignmentEditor extends React.Component {
    ...
    makeTaskParameterTemplates(){
        ...
        let createTaskObject = function(taskOptionsObject) {
            let newTask = {
                TA_display_name: '',
                TA_type: '',
                TA_documentation:'',
    ...
    UNSAFE_componentWillMount() {
        ...
        this.props.__(this.state.Strings, (newStrings) => {
            ...
            if (this.props.CourseID === '*' || this.props.CourseID === '') {
                ...
                axios.get(`/course/${this.props.CourseID}`).then((response) => {
                    const bod2 = response.data;
            ...
            if(this.props.PartialAssignmentID !== ''){
                ...
                axios.get(`/partialAssignments/byId/${this.props.PartialAssignmentID}?userId=${assignmentOptions.userId}&courseId=${assignmentOptions.courseId}`).then((response) => {
                    const assignBody = response.data;
                    if(response.status !== 200 || assignBody == null || assignBody.PartialAssignment == null || assignBody.PartialAssignment.Data == null){
                        return;
                    }
    ...
    onSave(){
        ...
        axios.post('/assignment/save', options).then((response) => {
            const body = response.data;
    ...
    onSubmit() {
        ...
        */
        ...
        axios.post(`/assignment/create?assignment=${o.assignment}&userId-${o.userId}&partialAssignmentId=${o.partialAssignmentId}&courseId=${o.courseId}&saveData=${o.saveData}`)
            .then((res) => {
            if (res.status === 200) {
                document.body.scrollTop = document.documentElement.scrollTop = 0;
                //Todo show message
    ...
    addConsolidation(stateData, parentIndex, workflowIndex) {
        ...
        let selectedNode = stateData[workflowIndex].WorkflowStructure.first(function(node) {
    ...
    canConsolidate(taskIndex,workflowIndex, isReflect){
        ...
        return this.state.WorkflowDetails[workflowIndex].Workflow[selectedNode.children[this.CONSOL_DISP_IDX].model.id].TA_type === TASK_TYPES.NEEDS_CONSOLIDATION;
    }
    ...
    canDispute(taskIndex,workflowIndex, isReflect){
        ...
        if(selectedNode.children.length === 0 || selectedNode.children[this.CONSOL_DISP_IDX] === undefined || selectedNode.children[this.CONSOL_DISP_IDX].model.id === -1){
            ...
            else return this.state.WorkflowDetails[workflowIndex].Workflow[selectedNode.children[this.CONSOL_DISP_IDX].children[this.CONSOL_DISP_IDX].children[this.CONSOL_DISP_IDX].model.id].TA_type === TASK_TYPES.DISPUTE;
        }
    }
    ...
    getAssessIndex(parentIndex, workflowIndex, stateData) {
        ...
        let selectedNode = workflowData[workflowIndex].WorkflowStructure.first(function(node) {
            return node.model.id === parentIndex;
        });
    ...
    hasConsolidate(taskIndex, workflowIndex){
        ...
        return this.state.WorkflowDetails[workflowIndex].Workflow[consolIndex].TA_type === TASK_TYPES.CONSOLIDATION;
    }
    ...
    getParentTask(taskIndex, workflowIndex, stateData){
        let index = this.getParentIndex(taskIndex, workflowIndex, stateData);
        if(stateData === undefined ){
            stateData = this.state.WorkflowDetails;
        }
    ...
    changeDefaultFieldReferences(taskIndexBeingRemoved, fieldIndexBeingRemoved, workflowIndex, stateData){
        let newData = stateData || this.state.WorkflowDetails;
        let taskListToCheck = this.getAlreadyCreatedTasks(taskIndexBeingRemoved, workflowIndex, stateData);

        taskListToCheck.forEach(function(taskInfo){
            let task = newData[workflowIndex].Workflow[taskInfo.value];

            Object.keys(task.TA_fields).forEach(function(key){
                if(isNaN(key)){
                    return;
                }
                let defaultRefersTo = task.TA_fields[key].default_refers_to;
                if( defaultRefersTo !== null && defaultRefersTo[0] !== null){
                    if(defaultRefersTo[0] === taskIndexBeingRemoved){
                        let savedIndex = typeof defaultRefersTo[1] == 'string' ? parseInt(defaultRefersTo[1]) : defaultRefersTo[1];
                        let removingIndex = typeof fieldIndexBeingRemoved == 'string' ? parseInt(fieldIndexBeingRemoved) : fieldIndexBeingRemoved;
    ...
    isDefaultFieldRefersToToggled(fieldIndex,taskIndex, workflowIndex){
        return this.state.WorkflowDetails[workflowIndex].Workflow[taskIndex].TA_fields[fieldIndex].default_refers_to[0] != null;
    }
    ...
    render() {
        ...
        let submitButtonView = this.state.SubmitInProgress ? <div/> : (
            <PLButton onClick={this.onSubmit.bind(this)}>
                <i className="fa fa-check"/>{this.state.Strings.Submit}
            </PLButton>
        );
        let saveButtonView = (<PLButton onClick={this.onSave.bind(this)}>Save</PLButton>);
        ...
        if (!this.state.Loaded) {
            return (
                <div>
                    ...
                    <i style={{marginLeft: '45vw'}} className="fa fa-cog fa-spin fa-4x fa-fw"/>
                    ...
                    <TabPanel key={'tab ' + index}>
                        <div >
                            <ProblemDetailsComponent
                                key={'Workflows' + index} workflowIndex={index}
                            ...
                            <div className='task-details-section'>
                                ...
                                <div className='current-task-view'>
                                    <TaskDetailsComponent
                                        key={index + '-' + this.state.SelectedTask}
                ...
                <div className="editor-container">
                    ...
                    <div>
                        <AssignmentDetailsComponent
                            AssignmentActivityData={this.state.AssignmentActivityData}
